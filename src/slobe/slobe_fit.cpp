...
slobe::SLOBEResult slobe_fit(
    ...
    Eigen::VectorXd lambda_scaled(p);
    ...
    // X starts from Xinit, then center and scale
    ...
    if (!known_sigma) {
        sigma = std::sqrt((X * beta - y).squaredNorm() / (n - pstart));
    }
    ...
    if (family == "gaussian") {
      alpha = 1.0 / sigma;
      lambda_scaled = lambda * sigma; 
    } else if (family == "binomial") {
      alpha = 0.5;
      lambda_scaled = lambda * alpha;
    }
    ...
    double c = 0.0;
    if (sstart > 0.0) {
        double h = (sstart + 1.0) / sum_abs(beta) *
          (1.0 / (alpha * lambda(p - 1)));
        c = (h < 1.0) ? h : 1.0;
    } else {
        c = 1.0;
    }
    ...
    while (iter < max_iter) {
        ...
        div_X_by_w(X_div_w, X, w, n, p);

        Eigen::VectorXd lambda_sorted = lambda_scaled;
        std::sort(lambda_sorted.data(), lambda_sorted.data() + p, std::greater<double>());

        Eigen::VectorXd beta_hat =
          slope_libslope_fit(X_div_w, y, lambda_sorted, family);

        wbeta = beta_hat.cwiseAbs();
        argsort_desc(wbeta, order);
        
        // unweight
        for (int i = 0; i < p; ++i) beta_hat(i) /= w(i);
        beta_new = beta_hat;
        ...
        for (int k = 0; k < p; ++k) {
            int idx = order[k];
            b_sum += gamma[idx] * std::fabs(beta[idx]) * lambda[k];;
        }
        b_sum *= alpha;
        ...
        if (verbose) {
          if(family == "gaussian") {
            ...
            std::cout << "Error = " << error << " theta = " << theta
                      << " c = " << c << std::endl;
          }
        }
